...
const AttachmentsPage = () => {
    ...
    const [numAttachmentsPerPage, setNumAttachmentsPerPage] = useState<number>(6 * 3);
    ...
    const fetchPageSettings = async () => {
        ...
        if(retrievedNumAttachmentsPerPage !== null && retrievedNumAttachmentsPerPage > 5) setNumAttachmentsPerPage(retrievedNumAttachmentsPerPage);
    }
    ...
    return (
        <div className="w-full h-[calc(100vh-70px)] grid grid-rows-[auto,1fr] overflow-y-auto overflow-x-hidden p-4 gap-2 lg:p-8">
            <div className="w-full themed-root slide-in-top">
                ...
                <div className="flex flex-row gap-1">
                    <input type="number" className="themed-input w-16" max={6 * 3} min={1} placeholder="Items per page" title="Items per page" value={numAttachmentsPerPage} onChange={(e) => setNumAttachmentsPerPage(parseInt(e.target.value))}/>
                    <div className="construct-search-bar col-span-2">
                        <input placeholder="Search" value={search} onChange={(e) => setSearch(e.target.value)}/>
                    </div>
                    <button className="themed-button-pos" onClick={() => fetchAttachments()} title="Refresh Attachments"><RefreshCcw size={'1.5rem'}/></button>
                    <button className="themed-button-pos" onClick={() => {page > 1 && setPage(page - 1)}} title="Previous Page"><ArrowBigLeft size={'1.5rem'}/></button>
                    ...
                    <button className="themed-button-pos" onClick={() => {page < numberOfPages && setPage(page + 1)}} title="Next Page"><ArrowBigRight size={'1.5rem'}/></button>
                </div>
            </div>
            <div className={`w-full h-11/12 grid grid-cols-6 grid-rows-[auto,2fr] gap-2`}>
                {filteredAttachments.slice(currentAttachmentsStart, currentAttachmentsEnd).map((attachment) => {
                    ...
                    if(attachment.fileext === "mp3" || attachment.fileext === "wav" || attachment.fileext === "flac"){
                        return (
                            <div className="col-span-1 w-full h-full themed-root flex flex-col justify-center items-center gap-2 pop-in">
                                <p className="text-center font-semibold">{attachment.name}</p>
